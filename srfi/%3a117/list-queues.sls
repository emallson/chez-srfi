(library (srfi :117 list-queues (1))
  ...
  (import (except (rnrs)
                  assoc filter find fold-right
                  for-each map member partition remove)
          (srfi :1 lists))
;;; R7RS shims.  Comment these out on an R7RS system.
;;; TFA stole this code from Chibi Scheme, which is BSD-licensed.
  ...
  (define (list-set! ls k x)
    (cond ((null? ls) (error 'list-set! "invalid list index" ls))
          ((zero? k) (set-car! ls x))
